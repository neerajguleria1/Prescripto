  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-200 backdrop-blur-sm sticky top-0 bg-white/80 z-50 shadow-sm' >
      <NavLink to={'/'} className='flex items-center gap-2'>
        <img className='w-44 cursor-pointer hover:scale-105 transition-transform duration-300' src={assets.logo} alt="" />
      </NavLink>
      ...
      <div className='flex items-center gap-4'>
        {
          user ?
            ...
            : <button onClick={() => navigate('/login')} className='py-3 px-8 bg-gradient-to-r from-[#5f6FFF] to-[#7a88ff] text-white rounded-full font-medium cursor-pointer hover:shadow-lg hover:scale-105 transition-all duration-300'>Create Account</button>
        }
        ...
        <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-300`} >
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            <NavLink onClick={() => setShowMenu(false)} to='/' className='w-full'> <p className='px-4 py-3 rounded-lg hover:bg-[#5f6FFF] hover:text-white transition-all duration-300 text-center'>HOME</p> </NavLink>
